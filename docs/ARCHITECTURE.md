...
| `jetpacs-demo.el` | onboarding: the first-run Start tab (teaches the M-x button, retires once the tour exists) + `jetpacs-setup-demo`, which writes the two-file guided tour (`walkthrough.org`, `hello-app.el`) into `~/jetpacs-demo/` and opens it |
