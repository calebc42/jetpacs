...
;; first run copies it in from /sdcard/Documents (staged by the companion's
;; onboarding); after that it self-updates, so this block never changes
;; again.  Keep it near the TOP of init.el, so an error later in your own
;; config can't stop the bridge from booting.
