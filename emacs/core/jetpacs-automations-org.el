;;; jetpacs-automations-org.el --- Automations as literate org -*- lexical-binding: t; -*-
...
(defcustom jetpacs-automations-org-file nil
...
(defvar jetpacs-automations-org--ids nil
  "Trigger ids registered from the org file (replaced on each reload).")

(defun jetpacs-automations-org--file ()
  (or jetpacs-automations-org-file
      (expand-file-name "automations.org" org-directory)))

;; ─── Parsing ─────────────────────────────────────────────────────────────────

(defconst jetpacs-automations-org--types jetpacs-triggers-supported-types
...
(defun jetpacs-automations-org--parse-trigger (str)
...
goes in `:PARAMS:'.  Returns nil for an empty or unknown-type string."
  (pcase-let* ((tokens (split-string (or str "") "[ \t]+" t))
               (`(,type . ,rest) tokens))
    (when (and type (member type jetpacs-automations-org--types))
      (cons type
            (pcase type
...
(defun jetpacs-automations-org--read (str)
...
(defun jetpacs-automations-org--handler (src headline)
           ...
           (lambda (hl)
             (when-let ((trigger (org-element-property :TRIGGER hl)))
               (let* ((headline (org-element-property :raw-value hl))
                      ...
                      (parsed (jetpacs-automations-org--parse-trigger trigger)))
                 (cond
                  ...
                  (t
                   (let* ((src (car (org-element-map hl 'src-block
                          ...
                          (params (or (jetpacs-automations-org--read
                     ...
                     (push (list :id (format "org/%s" headline)
                                 ...
                                 (when-let ((th (org-element-property
                                                 :THROTTLE hl)))
                                   (string-to-number th))
                                 :on-fire (jetpacs-automations-org--read
                                           (org-element-property :ON_FIRE hl))
                                 :handler (when src
                                            (jetpacs-automations-org--handler
                                             src headline)))
                           rules)))))))))))
    (nreverse rules)))
...
(defun jetpacs-automations-org-reload ()
...
the file is the source of truth for the `org/' id namespace."
  (interactive)
  (let* ((rules (jetpacs-automations-org--rules))
    ...
    ;; The loader owns its rules under its own name, so app teardown
    ;; never sweeps org-authored triggers.
    (with-jetpacs-owner "org-automations"
      ;; Unregister leavers first, then (re)register — each call pushes,
      ;; and replace-set makes the intermediate states harmless.
      (dolist (stale (cl-set-difference jetpacs-automations-org--ids ids
                                        :test #'equal))
    ...
    (setq jetpacs-automations-org--ids ids)
...
(defun jetpacs-automations-org--after-save (file)
  "Reload when the phone saves the automations FILE."
  (when (equal (expand-file-name file)
               (expand-file-name (jetpacs-automations-org--file)))
    (jetpacs-automations-org-reload)))

(with-eval-after-load 'jetpacs-files
  (add-hook 'jetpacs-files-after-save-hook #'jetpacs-automations-org--after-save))

;; Load rules when the file exists; a missing file is simply zero rules.
(when (file-readable-p (jetpacs-automations-org--file))
  (jetpacs-automations-org-reload))

(provide 'jetpacs-automations-org)
;;; jetpacs-automations-org.el ends here
