 ...
 * built by `jetpacs-widget-item' / `jetpacs-widget-divider' Emacs-side, actions
 ...
 */
class JetpacsWidgetListService : RemoteViewsService() {
    override fun onGetViewFactory(intent: Intent): RemoteViewsFactory =
        WidgetListFactory(
            applicationContext,
            intent.data?.schemeSpecificPart ?: JetpacsWidgetProvider.SURFACE)
}
...
private class WidgetListFactory(
    ...
    override fun getViewAt(position: Int): RemoteViews {
        val item = items.getOrNull(position)
            ?: return RemoteViews(context.packageName, R.layout.widget_jetpacs_agenda_divider)
        return if (item.has("divider")) {
            RemoteViews(context.packageName, R.layout.widget_jetpacs_agenda_divider).apply {
    ...
    private fun rowView(item: JSONObject): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_jetpacs_agenda_item)
