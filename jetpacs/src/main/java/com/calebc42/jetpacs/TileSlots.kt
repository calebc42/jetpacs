...
/**
 * Blank Quick Settings tile slots, the tile twin of [JetpacsCustomWidgetProvider]:
 * five pre-registered tiles composed entirely from Emacs by pushing a
 * `tile:customN` surface built with `jetpacs-tile'. The services are declared
 ...
 */
object JetpacsTileSlots {
    val SLOTS: Map<String, Class<out JetpacsTileSlotService>> = mapOf(
        "tile:custom1" to JetpacsTile1::class.java,
        "tile:custom2" to JetpacsTile2::class.java,
        "tile:custom3" to JetpacsTile3::class.java,
        "tile:custom4" to JetpacsTile4::class.java,
        "tile:custom5" to JetpacsTile5::class.java)
    ...
    private const val TAG = "JetpacsTiles"
}

abstract class JetpacsTileSlotService : TileService() {
        ...
        qsTile?.apply {
            if (spec == null) {
                ...
                icon = Icon.createWithResource(
                    this@JetpacsTileSlotService, tileIcon(spec.optString("icon")))
            }
            updateTile()
        }
    }
...
class JetpacsTile1 : JetpacsTileSlotService() { override val surface = "tile:custom1" }
class JetpacsTile2 : JetpacsTileSlotService() { override val surface = "tile:custom2" }
class JetpacsTile3 : JetpacsTileSlotService() { override val surface = "tile:custom3" }
class JetpacsTile4 : JetpacsTileSlotService() { override val surface = "tile:custom4" }
class JetpacsTile5 : JetpacsTileSlotService() { override val surface = "tile:custom5" }