...
internal fun widgetManager(context: Context): SurfaceManager =
    JetpacsRuntime.surfaceManager ?: SurfaceManager(context)
...
fun renderWidgetSurface(context: Context, record: SurfaceRecord) {
    when {
        record.surface == JetpacsWidgetProvider.SURFACE ->
            JetpacsWidgetProvider.renderAll(context, record)
        record.surface in JetpacsCustomWidgetProvider.SLOTS ->
            JetpacsCustomWidgetProvider.renderAll(context, record)
        else -> Log.w("JetpacsWidgets", "No widget renderer for '${record.surface}'")
    }
}
 ...
 * `widget:customN` surface (rows built with `jetpacs-widget-item' /
 * `jetpacs-widget-divider'); the companion just renders. Multi-view specs get
 * the same header dropdown as the agenda widget.
 */
abstract class JetpacsCustomWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        val SLOTS: Map<String, Class<out JetpacsCustomWidgetProvider>> = mapOf(
            "widget:custom1" to JetpacsCustomWidget1::class.java,
            "widget:custom2" to JetpacsCustomWidget2::class.java,
            "widget:custom3" to JetpacsCustomWidget3::class.java,
            "widget:custom4" to JetpacsCustomWidget4::class.java,
            "widget:custom5" to JetpacsCustomWidget5::class.java)
...
class JetpacsCustomWidget1 : JetpacsCustomWidgetProvider() { override val surface = "widget:custom1" }
class JetpacsCustomWidget2 : JetpacsCustomWidgetProvider() { override val surface = "widget:custom2" }
class JetpacsCustomWidget3 : JetpacsCustomWidgetProvider() { override val surface = "widget:custom3" }
class JetpacsCustomWidget4 : JetpacsCustomWidgetProvider() { override val surface = "widget:custom4" }
class JetpacsCustomWidget5 : JetpacsCustomWidgetProvider() { override val surface = "widget:custom5" }